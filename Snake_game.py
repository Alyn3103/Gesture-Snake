...
pygame.init() #    intilizing pygame windows

Red = (244, 30, 74)
Green = (255, 255, 255)
Blue = (1, 1, 1)    #initilzing basic colors

border = (23, 234, 223)  # intilizing a border color for uniqueness


Snake_Window = pygame.display.set_mode([600,600])  #setting size of windows 
pygame.display.set_caption("Snake_game")     #setting name of window

pygame.display.update()   #updating the windows
Clock = pygame.time.Clock()  # used for Refresh rate

Score_font = pygame.font.SysFont(None, 30)   #setting font for score using system font iteself , None is for color of font

def write_score(text,color,x,y):    # 
    screen_text = Score_font.render(text,True,color) #text for score
    Snake_Window.blit(screen_text,[x,y]) #to display score on screen
...
def Game_loop():
    ...
    while not game_exit:
        if game_over == True:
                ...
                for landmarks in output.multi_hand_landmarks:
                    for id , pos in enumerate(landmarks.landmark):
                        ...
                        if id == 8:
                            ...
                            cv2.circle(image, (pos_x, pos_y), 25, border, cv2.FILLED)
            ...
            if Snake_head_x<0 or Snake_head_x>565 or Snake_head_y<52.5 or Snake_head_y>547.5:
                game_over = True
            ...
            pygame.draw.rect(Snake_Window, border, [0, 597.5, 600, 2.5]) #down
            pygame.draw.rect(Snake_Window, border, [597.5, 0, 2.5, 600])  #right
            pygame.draw.rect(Snake_Window, border, [0, 0, 2.5, 600])  #left
            
            pygame.draw.rect(Snake_Window, border, [0, 50, 600, 2.5]) #Up
            pygame.draw.rect(Snake_Window, border, [0, 0, 600, 2.5])  #up middle
